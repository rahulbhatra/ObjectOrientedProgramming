...
def max_amazon_servers(powers: List) -> int:
    ...
    for i in range(len(unique_powers)):
        ...
        else: # either start power or single power (eg. 1, 2) for (1, 4) just assign the power to local_max_server
             local_max_server = count[power]
...
print(max_amazon_servers([7, 1, 2, 2, 3, 4, 2, 2])) # expected 1, 2, 2, 3, 2, 2
